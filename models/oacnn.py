...
"""
Implementation of OACNNs from the repo: https://github.com/Pointcept/Pointcept
"""
...
class _OACNNs(nn.Module):
    def __init__(
        ...
        self.final = spconv.SubMConv3d(
            dec_channels[0], num_classes, kernel_size=1, algo=algo)
        self.apply(self._init_weights)

        # nn.init.constant_(self.final.bias, -2.0)

    def forward(self, input: dict | torch.Tensor | spconv.SparseConvTensor, data: dict = {}) -> spconv.SparseConvTensor:
        if isinstance(input, spconv.SparseConvTensor):
            ...
            x = to_sparse(input, "spconv")
        else:
            raise ValueError(
                f"input_dict should be dict, torch.Tensor or SparseConvTensor, but got {type(input)}"
            )
...
class OACNNsInterleaved(_OACNNs):
    def __init__(self, in_channels=1, classes=1, r=2, backend_type="spconv", depth=3):
        ...
        # TEMP: for speed evaluation
        # self.interleaver = torch.compile(Interleaver(self.r))
        # self.deinterleaver = torch.compile(Deinterleaver(self.r))
        self.apply(self._init_weights)

    def forward(self, input, data={}):
        x = self.interleaver(input)
        # x = to_sparse(x, "spconv")
        out = super().forward(x, data)
        # out = to_dense(out, input.shape)
