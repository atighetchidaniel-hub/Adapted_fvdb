...
class SparseWeightedDiceLoss(nn.Module):
    ...
    def dice(self, input, target: torch.Tensor, weight: float | None = None) -> torch.Tensor:
        ...
        fn = (((1 - input_feat) ** 2) * (target_feat ** 2)).sum(dim=0)

        if weight is not None:
            intersect = weight * intersect

        denominator = (2 * intersect) + (self.alpha * fp) + ((1 - self.alpha) * fn)

        if self.debug:
            print(f"intersect: {intersect}, fp: {fp}, fn: {fn}, denominator: {denominator}")
